...
public:
    ...
    void encodeMessage(string msg, string messagePath) 
    {
        ...
        {
            //uint16_t randomXPosition = rand() % _x + 0;
            ...
            for (int j = randomXPosition; (j < img.cols) && (k < s.length()); j++)
            {
               ...
               // _MessageMatrix[i][j].val[1] = v.val[1];
               // _MessageMatrix[i][j].val[2] = v.val[2];
                k++;
            } 
    ...
    //    showMatrixes();
       ...
       /* Mat test = imread(_MessageImagePath, IMREAD_UNCHANGED);
        for (int i = 0; i < _y; i++)
        {
            for (int j = 0; j < _x; j++)
            {
                cout << "XIm: " << j << ", YIm: " << i << ",B:" << int(test.at<Vec3b>(i,j).val[0]) << ",G:" << int(test.at<Vec3b>(i, j).val[1]) << ",R:" << int(test.at<Vec3b>(i, j).val[2]) << endl;
            }
        }
        */
    }
        ...
        for (int i = 0; i < _y; i++) 
        {
            for (int j = 0; j < _x; j++)
            {
               ...
               /*
                ...
                Vec3b Nbuf;
                Nbuf.val[0]= NewImage.at<Vec3b>(i, j).val[0];
                Nbuf.val[1] = NewImage.at<Vec3b>(i, j).val[1];
                Nbuf.val[2] = NewImage.at<Vec3b>(i, j).val[2];
                _NewMatrix[i][j].val[0] = Nbuf.val[0];
                _NewMatrix[i][j].val[1] = Nbuf.val[1];
                _NewMatrix[i][j].val[2] = Nbuf.val[2];
                //_NewMatrix[i][j][1] = bufff[1];
               // _NewMatrix[i][j][2] = bufff[2];*/
            }
        }
        //showMatrixes();
	}
